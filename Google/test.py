...
def filter_and_sort_recent_reviews(reviews, months=5):
    ...
    for review in reviews:
        ...
        if int(match.group()) <= months:
            recent_reviews.append(review)
    return recent_reviews


# JSON 파일 기본 리뷰 5개씩만 넘어옴
def get_place_reviews(api_key, place_id, max_reviews=10):
    url = f"https://maps.googleapis.com/maps/api/place/details/json?place_id={place_id}&fields=reviews&language=ko&reviews_sort=newest&key={api_key}"
...
if __name__ == "__main__":
    ...
    df = pd.read_excel('restaurants_with_place_id.xlsx')
    #place_id = df['Place ID']
    place_id = 'ChIJ1RG1J6vkDDURux7pPbxUvbY'

    reviews = get_place_reviews(api_key, place_id, max_reviews=10)
